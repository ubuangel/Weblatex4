   ...
   while (command != 'q' && command != 'Q') {
      switch (command) {
         ...
         default:
            ...
            printf("Porfavor intentelo denuevo\n");
      }
      command = Get_command();
   }
   Free_list(&head_p);
...
int  Member(int value, struct list_node_s* head_p) {
   ...
   if (curr_p == NULL || curr_p->data > value) {
      printf("%d el valor no esta en la lista\n", value);
      return 0;
   } else {
...
char Get_command(void) {
   ...
   printf("Porfavor ingrese un comando p ,i ,m o q para salir  ");
...
int  Get_value(void) {
   ...
   printf("Porfavor ingrese un valor:  ");
   scanf("%d", &val);
   return val;
}  /* Get_value */
